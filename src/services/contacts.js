...
import { SORT_ORDER } from '../constants/index.js'; //константа з порядком сортування
...
export const getContactById = async (contactId, userId) => {
  const contact = await ContactsCollection.findOne({ _id: contactId, userId }); //знаходить документ за його ідентифікатором
...
export const deleteContact = async (contactId, userId) => {
  const contact = await ContactsCollection.findOneAndDelete({
    _id: contactId,
    userId,
  }); //знаходить і видаляє документ за його ідентифікатором
